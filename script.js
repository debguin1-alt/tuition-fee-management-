...
function displayMenu() {
  ...
  deb_guin.teachers.forEach((t, i) => {
    ...
    const lastPaidText = t.last_paid_month
      ? `${getMonthName(t.last_paid_month)} ${t.last_paid_year}`
      : "Never";

    html += `
      <div class="teacher">
        <div><span class="teacher-name">${i + 1}. ${t.name}</span> <span class="teacher-subject">(${t.subject})</span></div>
        <div class="teacher-info">Last Paid: ${lastPaidText}</div>
        <div class="teacher-info">Due: ${dueMonths} months = Rs.${total}</div>
      </div>`;
  });
